                ...
                <div class="profile">
                    ...
                    <div class="create-post">
                        ...
                        <form @submit.prevent>
                            ...
                            <button @click="createPost" :disabled="post.content == ''" class="button">Crear</button>
                        </form>
                    </div>
                </div>
            </div>
            <div class="col2">
                <transition name="fade">
                    <div v-if="hiddenPosts.length" @click="showNewPosts" class="hidden-posts">
                        <p>
                            Click para ver más posts <span class="new-posts">{{ hiddenPosts.length }}</span>
                            <span v-if="hiddenPosts.length > 1">Nuevos posts</span><span v-else>Nuevo post</span>
                        </p>
                    </div>
                </transition>
                <div v-if="posts.length">
                    <div v-for="post in posts" class="post" :key="post.id">
                        ...
                        <ul>
                            <li><a @click="openCommentModal(post)">Comentarios {{ post.comments }}</a></li>
                            <li><a @click="likePost(post.id, post.likes)">likes {{ post.likes }}</a></li>
                            <li><a @click="viewPost(post)">Ver el post completo</a></li>
                        </ul>
                    </div>
                </div>
                <div v-else>
                    <p class="no-results">No hay posts</p>
                </div>
            </div>
        </section>
            ...
            <div v-if="showCommentModal" class="c-modal">
                <div class="c-container">
                    ...
                    <form @submit.prevent>
                        ...
                        <button @click="addComment" :disabled="comment.content == ''" class="button">Agregar comentario</button>
                    </form>
                </div>
            </div>
        </transition>
